(() => {
  ...
  var Bgm = class {
    ...
    pad(freq, start, dur, vol) {
      ...
      gain.gain.setValueAtTime(vol * 0.85, start + dur - 1);
    ...
    playStep() {
      ...
      this.step += 1;
      chord.forEach((f, i) => this.pad(f, t, this.chordDur, 0.032 - i * 5e-3));
      for (let i = 0; i < 3; i++) {
        this.sparkle(chord[i % chord.length] * 2, t + 1 + i * 1.4, 0.018);
      }
      this.timer = setTimeout(() => this.playStep(), (this.chordDur - 0.15) * 1e3);
    }
    start() {
